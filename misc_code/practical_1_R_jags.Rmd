...
You can plot the data using the ```plot()``` function. 
...
- Apply a similar window to an AR1 process (with drift). Do you see something similar?
 
*** 
## Removing Seasonality

- Load the HadCET data
- Plot an ACF/PACF
- Remove the seaonality, and do the same
...
- Use the ```plot()```, print() and ```trace_plot()``` functions on your ```model_run``` variable to check the output.
